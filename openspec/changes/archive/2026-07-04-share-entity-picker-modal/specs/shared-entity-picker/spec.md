...
The shared picker SHALL resolve every list avatar through `getAvatarUrl(entity.imageUrl)`, with the ui-avatars fallback applied on image error. No picker SHALL pass a raw local asset path to `<img src>`.
