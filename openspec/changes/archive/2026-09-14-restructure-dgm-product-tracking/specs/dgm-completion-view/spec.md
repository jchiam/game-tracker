...
`CompletionView` (`src/pages/digimon/components/CompletionView.tsx`) SHALL render, from `trackedProducts` and `ALL_PRODUCTS` alone, an overall row followed by one row per `DGM_LINES` entry in that order. Each row SHALL show the line name, `productsOwned / productsTotal`, the integer product percentage, a secondary readout `variantsOwned / variantsTotal variants`, and a `.completion-bar` whose `.completion-bar-fill` width equals the product percentage and uses the shared progress gradient. A product counts as **owned** when `deriveOwnership` is `owned`; a variant counts when its state is `owned`; wishlist and interested SHALL NOT count. Totals are catalog counts for the line. The view SHALL NOT fetch or persist anything and SHALL render no product cards.

#### Scenario: Owned counts per line

- **WHEN** the catalog has 5 Pendulum products with 12 variants and the user owns variants of 2 products (3 variants) and wishlists a third
- **THEN** the Pendulum row reads `2 / 5`, `40%`, `3 / 12 variants`, with fill width `40%`
