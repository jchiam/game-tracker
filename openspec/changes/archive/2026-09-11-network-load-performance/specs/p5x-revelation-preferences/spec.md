...
- **THEN** the existing rows are deleted and the new rows inserted inside the single `replace_preference_rows` RPC, so a failure leaves the previous rows in place (see shared-save-behaviour)
