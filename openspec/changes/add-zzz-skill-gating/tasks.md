...
- [x] 1.1 Write `supabase/migrations/20260910000000_zzz_skill_progress.sql`: add
...
- [x] 2.1 `src/types.ts`: on `ZzzTrackedAgent`, replace the five `skill*Maxed` booleans with
...
- [x] 3.1 `agentService.ts`: column map gains `skillProgress: 'skill_progress'`, drops the five
      flag entries; select list and insert defaults (`skill_progress: 0`) updated; load mapping
      `skillProgress: row.skill_progress ?? 0`
- [x] 3.2 `agentService.test.ts`: replace flag-mapping tests with skill-progress load/default/patch
      wiring tests

## 4. Hook

- [x] 4.1 `useAgents.ts`: drop the five flag updaters; add
      `updateSkillProgress = makeFieldUpdater('skillProgress', { clamp: [0, 2] })`; add default
      `skillProgress: 0`; export swap
- [x] 4.2 `useAgents.test.ts`: update updater and default-shape tests

## 5. Card

- [x] 5.1 `agentBadges.ts`: remove `ZZZ_COMBAT_SKILLS`, `ZzzSkillKey`, `ZzzSkillField`
- [x] 5.2 `AgentCard.tsx`: replace the `ToggleChips` skills row with the P5X-pattern
...
- [x] 5.3 `AgentCard.tsx`: replace the `Skl n/5` summary chip with the conditional indicator —
      "Skills ✓" at 2, "🐹 Gated" at 1, no chip at 0, styled from `getProgressStyle(value, 0, 2)`
- [x] 5.4 `AgentCard.test.tsx`: update section, chip, and callback tests

## 6. Page

- [x] 6.1 `ZzzPage.tsx`: add `passGateFilter` state; compose the predicate
...
- [x] 6.2 `ZzzPage.tsx`: render `filterRow` with the "🐹 Gated" chip, accent
      `--color-zzz-rarity-s`; filter-specific `noMatchMessage`
- [x] 6.3 `ZzzPage.test.tsx`: filter chip narrows/restores, composes with search, empty state,
      held ghost-tag
