...
- **THEN** only `'Flat ATK & Flat DEF'` is available

### Requirement: Shared substat pool

The system SHALL export a `SUB_STATS` array containing all valid substat types: Flat ATK, ATK%, Flat DEF, DEF%, Flat HP, HP%, DMG Multiplier%, Ailment Accuracy%, Crit Rate%, Crit Multiplier%, Speed, SP Recovery%, Pierce Rate%.
