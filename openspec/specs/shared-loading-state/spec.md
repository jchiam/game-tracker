...
`PartiesView` SHALL accept an optional `isInitialLoad` boolean (default `false`). When true (and a session exists), the view SHALL render `LoadingState` — labelled `Loading your {nouns.partiesLower}…` — in place of the parties grid contents, before the empty-parties check, while keeping the header and create button visible. Each game's `PartiesTab` adapter SHALL forward the roster hook's existing `isInitialLoad` flag.

#### Scenario: Initial load no longer shows false empty

- **WHEN** a signed-in user opens the Lineups tab while the initial DB load is in flight
...
- **WHEN** the initial load has completed and the user has no parties
