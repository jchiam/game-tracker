# dgm-game-progress Specification

## Purpose

In-game progress on a tracked Digimon product: the guide-driven checklist semantics (tracks, items, prerequisites, overall formula), the progress array on the product row, and the owned-only gating of every progress surface.
