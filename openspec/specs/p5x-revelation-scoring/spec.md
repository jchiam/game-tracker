...
Revelation evaluation algorithm for Persona 5: The Phantom X. Scores a Thief's five equipped
revelation cards against `revelationPreferences` across three N2E-parity terms — emergent set
composition (`setTerm × 0.35`), main stat (`× 0.30`), and sub stats (`× 0.35`) — producing a
0–100 score, or `-1` for insufficient data. The set term is unique among the games: it scores the
whole five-card hand (graded Heavens breakpoint progress + gated Space match) rather than any one
card. Grade thresholds match N2E's scale, duplicated pending the cross-game scoring-alignment
change.
