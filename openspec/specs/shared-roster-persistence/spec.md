...
Service-layer persistence core shared by all game modules via `createRosterPersistence` (`src/services/rosterPersistence.ts`). Covers config-driven CRUD against per-game Supabase tables, DB-disabled early-return semantics, catalog merge on load, profile upsert on insert, patch-to-column mapping, the extras seam for game-specific joined-table reconstruction, and the shared preference-rows save pattern.
