...
Automated catalog regeneration and ImageKit asset upload for Arknights: Endfield,
bringing AE onto the shared per-game update pattern (Update Pipeline in `CONTEXT.md`).
`scripts/update-ae-data.mjs` fetches the operator list from Prydwen's Gatsby page-data
endpoints (via the CloudFront origin), regenerates the operator Catalog, and uploads
portraits idempotently; a weekly GitHub Actions workflow opens an auto-PR when the run
produced changes. Supersedes the hand-authored catalog and one-shot seed script from
Phase 1. The AE weapon catalog (`weapons.ts`) remains hand-authored per the
`ae-weapon-catalog` spec until a structured weapon source exists.
