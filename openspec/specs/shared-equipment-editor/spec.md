...
The cross-game equipment editor interaction contract shared by every equipment-tracking game (HSR relics, N2E cartridge, P5X revelations): the clickable slot grid in the card's edit body, the single unified all-slots editor modal with an anchor-slot scroll, the complete Build Preferences tab composition (set selects, per-slot main-stat chains, substat chain, comments), per-slot clear semantics (set "None" clears in multi-slot editors; footer "Un-equip" only in single-item editors), the read-only Target Build card readout, and the single numeric score surface (header `ScoreBadge` only).
