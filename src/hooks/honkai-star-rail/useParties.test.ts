...
import type { Party, PartySaveResult } from '@/types';
...
describe('useParties', () => {
  ...
  describe('initial load', () => {
    it('loads parties from DB when session is present', async () => {
      mockLoadParties.mockResolvedValue([makeParty('p1', 'Alpha Team')]);
      ...
      expect(result.current.parties[0].name).toBe('Alpha Team');
    });
  ...
  describe('saveParty', () => {
    it('creates a new party and refreshes the list', async () => {
      ...
      mockLoadParties.mockResolvedValueOnce([makeParty('new-party-id', 'New Team')]);

      await act(async () => {
        await result.current.saveParty({ name: 'New Team', members: [] });
      });
      ...
      expect(result.current.parties).toHaveLength(1);
      expect(result.current.parties[0].name).toBe('New Team');
    });
    ...
    it('returns null and does not call DB when session is absent', async () => {
      ...
      let returned: PartySaveResult | null = 'sentinel' as any;
      await act(async () => {
