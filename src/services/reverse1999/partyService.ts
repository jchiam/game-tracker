...
const persistence = createPartyPersistence<Party, PartyMember>({
  ...
  memberFromRow: (row) => ({ entityId: row.arcanist_id, slotIndex: row.slot_index }),
  memberToRow: (member) => ({ arcanist_id: member.entityId, slot_index: member.slotIndex }),
  ...
  // writing it here would reset a favorited lineup on every edit. Favorite
