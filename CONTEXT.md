...
| Digimon Virtual Pets     | `dgm`    | `digimon`               | **device**          | Collection modality (see Tracker Modality): one digivice / virtual-pet release per catalog entry (`line`, `series`, `releaseYear`, `region`, `colorway`; `DGM_LINES` = distinct lines in catalog order); tracked rows carry ownership status (owned / wishlist), condition (sealed / boxed / loose / none), acquisition date, notes; the tracked set is the **collection**; second view is **Completion**; no parties, equipment, or scoring; catalog is seed-generated (see Update Pipeline) |
