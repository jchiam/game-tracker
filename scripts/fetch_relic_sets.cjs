...
https.get('https://raw.githubusercontent.com/Mar-7th/StarRailRes/master/index_new/en/relic_sets.json', { headers: { 'User-Agent': 'Node.js' } }, (res) => {
  let body = '';
  ...
  res.on('end', async () => {
    try {
      ...
      for (const [id, info] of Object.entries(data)) {
        ...
        if (info.icon) {
          downloadTasks.push(
            downloadImage(`${IMAGE_BASE_URL}${info.icon}`, dest)
              .catch(e => console.error(`Skipped icon ${id} - ${e.message}`))
          );
        }
      }
