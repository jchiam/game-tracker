...
2. **Service layer** (`src/services/{game}/`): Thin config adapter over the shared roster-persistence core (`src/services/rosterPersistence.ts`). Each game's `{entity}Service.ts` calls `createRosterPersistence(config)` — config supplies table name, entity FK column, catalog array, patch-column map, insert defaults, select string, and a `fromRow(row, base)` mapper — and re-exports the produced functions as `load{Entities}FromDB`, `insert{Entity}`, `delete{Entity}`, `update{Entity}`. Game-specific joined-table reconstruction (HSR relics/build prefs, N2E cartridge prefs) plugs in via the optional `extras: { selectFragment, mapRow }` seam. Game-specific write functions (`upsertRelic`, `saveBuildPrefs`, `saveCartridgePreferences`, …) stay as per-game exports; preference-chain savers delegate to the shared `savePreferenceRows` helper. The core owns `DB_ENABLED` early-return, profile upsert, and error handling. Party service: same pattern — `partyService.ts` calls `createPartyPersistence(config)` (tables, default name, member row mappers, optional `extraSelect`/`extraFromRow`/`extraToRow` for `tier`/`is_favorited`) and re-exports `loadParties`, `saveParty`, `deleteParty` (+ `toggleFavoriteParty` for R1999/N2E). Party error semantics: `loadParties` throws (the shared hook catches); `saveParty` returns `null` on party-row failure and never rejects; `deleteParty`/`toggleFavoriteParty` return `false`.
